/**
 * Vercel Serverless Function: POST /api/analyze-report
 * Replicates backend/src/routes/analyze.js for Vercel deployment.
 * Requires env var: GEMINI_API_KEY
 * Optional env vars: SUPABASE_URL, SUPABASE_SERVICE_KEY (or SUPABASE_ANON_KEY)
 */
import { analyzeReportImage } from '../backend/src/lib/analyzeReportGemini.js'
import { extractLabsFromImage } from '../backend/src/lib/gemini.js'
import { getProfile, updateProfile } from '../backend/src/lib/db.js'
import {
  calculateRisk,
  calculateRiskWithLabs,
  buildRiskExplanation,
  scoreToLevel,
} from '../backend/src/utils/scoring.js'
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Primary: full AI analysis (score, summary, tips, key values)
    ...
    if (analysis.ok && analysis.data) {
      ...
      combinedReportSummary =
        data.summary ||
        'Your risk score above combines your questionnaire answers with the blood report values read by AI.'
      if (data.tips && data.tips.length) result.tips = data.tips
    } else {
      // Fallback: labs-only extraction
      if (analysis.error?.includes('API key') || analysis.error?.includes('GEMINI')) {
        return res.status(502).json({
          error:
            'AI blood report analysis is not configured. Add GEMINI_API_KEY to your Vercel environment variables.',
        })
      }
      try {
        ...
        combinedReportSummary =
          !quotaExceeded && (extractedLabs.hba1c != null || extractedLabs.fastingGlucose != null)
        ...
        console.error('analyze-report Gemini fallback:', geminiErr?.message)
        return res.status(502).json({
          error: geminiErr?.message || 'AI analysis failed. Check your API key and quota.',
        })
      }
    }
    ...
    if (profileId && !quotaExceeded && (extractedLabs.hba1c != null || extractedLabs.fastingGlucose != null)) {
      try {
        ...
        console.error('analyze-report updateProfile (non-fatal):', e?.message)
      }
    }
    ...
    const quotaMessage = quotaExceeded
      ? quotaReason === 'rate_limit'
        ? 'Rate limit reached (too many requests). Wait about a minute and try again.'
        : 'AI analysis is temporarily unavailable (quota exceeded). Your result is based on your answers only. Try again later.'
      : undefined
